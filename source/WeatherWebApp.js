...
//  use 01n.png instead of description for weather icon

/*
slide method:
css methods:
parent container position relative (or just body be position relative)
container <div> position absolute to make it work
@keyframe slide-in{
    from: top=100vh;
    to: top 0vh;
}
...
function myAnimationWeb() {
    ...
    document.getElementById("animationWeb").appendChild(imageArray[animationCountWeb]);
    document.getElementById("doppler_" + animationCountWeb).style.display = "inline";
...
function myAnimationTablet() {
    ...
    document.getElementById("animationTablet").appendChild(imageArray[animationCountWeb]);
    document.getElementById("doppler_" + animationCountWeb).style.display = "inline";
    document.getElementById("doppler_" + animationCountWeb).style.width = "80%";
...
function addToArray(newImage) {
    if (count < 10) {
        ...
        if (count >= 10) {
            ...
            // myAnimationWeb(); // run it immediately
            // myAnimationTablet(); // run it immediately

            // document.getElementById("arrImg").appendChild(imageArray[0]);
            // document.getElementById("doppler_0").style.display = "inline";
            // document.getElementById("arrImg").removeChild(imageArray[0]);
            
        }
    }
}

function tryToGetImage(dateObj) { // tries to get one image from the server
    ...
    newImage.onload = function () { // get the new image the puts it into the global array
...
function getTenImages() { // try to get 10 images
...
// function selectWeather(object){
//     let weather = object.weather[0].description;
//     switch(weather) {
//         case "clear sky":
//             if (time24h(object) >= 6 && time24h(object) < 20) {
//                 return "../assets/clearsky.svg";
//             }
//             return "../assets/clear-night.svg";
//         case "broken clouds":
...
function makeCorsRequest(city = 'Davis, CA, USA'){ // default city value
    let api = 'http://api.openweathermap.org/data/2.5/forecast/hourly?q=';
    let units = '&units=imperial';
    let key = '&APPID=563050648e6900dde8bce2f7e0471936';
    ...
    xhr.onload = function () {
        ...
        let calcRange = Math.pow(parsedLat - 38.5816, 2) + Math.pow(parsedLon + 121.4944, 2);
        let milesInDegree = 150 / 69;

        // console.log("this is parsed lat: ", parsedLat);
        // console.log("this is parsed lon: ", parsedLon);


        if (calcRange <= Math.pow(milesInDegree, 2)){ // math for latitude and longitude calculation within 150 miles radius
            ...
            document.getElementById("tabletCurrentTempId").textContent = setTemp1 + "°";
            ...
            alert("THE CITY SELECTED IS NOT WITHIN A 150 MILES RADIUS FROM SACRAMENTO. PLEASE TRY AGAIN (WEATHER ICONS AND TEMPERAURES ARE THAT OF THE LAST SEARCH)")
        }
